  ...
  class FormMessageField
    ...
    def self.openapi_types
      {
        ...
        :'min_size' => :'Integer',
        :'max_size' => :'Integer',
        :'options' => :'Array<FormOptionsInner>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SunshineConversationsClient::FormMessageField` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SunshineConversationsClient::FormMessageField`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@text.nil? && @text.to_s.length > 256
        invalid_properties.push('invalid value for "text", the character length must be smaller than or equal to 256.')
      end
      ...
      if !@options.nil? && @options.length > 200
        invalid_properties.push('invalid value for "options", number of items must be less than or equal to 200.')
      end
    ...
    def valid?
      ...
      return false if !@text.nil? && @text.to_s.length > 256
      ...
      return false if !@options.nil? && @options.length > 200
      true
    end
    ...
    def text=(text)
      if !text.nil? && text.to_s.length > 256
        fail ArgumentError, 'invalid value for "text", the character length must be smaller than or equal to 256.'
      end
    ...
    def options=(options)
      if !options.nil? && options.length > 200
        fail ArgumentError, 'invalid value for "options", number of items must be less than or equal to 200.'
      end
    ...
    def hash
      [typenamelabeltextemailselectplaceholdermin_sizemax_sizeoptions].hash
    end
