  ...
  class SwitchboardIntegration
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SunshineConversationsClient::SwitchboardIntegration` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SunshineConversationsClient::SwitchboardIntegration`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @deliver_standby_events.nil?
        invalid_properties.push('invalid value for "deliver_standby_events", deliver_standby_events cannot be nil.')
      end

      if !@message_history_count.nil? && @message_history_count > 10
        invalid_properties.push('invalid value for "message_history_count", must be smaller than or equal to 10.')
      end

      if !@message_history_count.nil? && @message_history_count < 1
        invalid_properties.push('invalid value for "message_history_count", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @deliver_standby_events.nil?
      return false if !@message_history_count.nil? && @message_history_count > 10
      return false if !@message_history_count.nil? && @message_history_count < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] message_history_count Value to be assigned
    def message_history_count=(message_history_count)
      if !message_history_count.nil? && message_history_count > 10
        fail ArgumentError, 'invalid value for "message_history_count", must be smaller than or equal to 10.'
      end

      if !message_history_count.nil? && message_history_count < 1
        fail ArgumentError, 'invalid value for "message_history_count", must be greater than or equal to 1.'
      end
    ...
    def hash
      [id, name, integration_id, integration_type, deliver_standby_events, next_switchboard_integration_id, message_history_count].hash
    end
