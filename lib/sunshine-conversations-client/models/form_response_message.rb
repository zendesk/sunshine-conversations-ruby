  ...
  class FormResponseMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SunshineConversationsClient::FormResponseMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SunshineConversationsClient::FormResponseMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @fields.length > 20
        invalid_properties.push('invalid value for "fields", number of items must be less than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if @fields.length > 20
    ...
    def fields=(fields)
      ...
      if fields.length > 20
        fail ArgumentError, 'invalid value for "fields", number of items must be less than or equal to 20.'
      end
